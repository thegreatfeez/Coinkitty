...
export default function TokenDetails(){
   ...
   const Chart = ({ data, type }) => {
       ...
       const maxPrice = Math.max(...data.map(d => type === 'candlestick' ? d.high : d.price));
       const minPrice = Math.min(...data.map(d => type === 'candlestick' ? d.low : d.price));
       ...
       for (let i = 0; i <= yAxisSteps; i++) {
           ...
           yAxisLabels.push({
               price: price,
               y: y,
               label: price >= 1000 ? `${(price/1000).toFixed(0)}K` : `${price.toFixed(0)}`
           });
       }
               ...
               <svg width="100%" height={chartHeight} viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="overflow-visible">
                   ...
                   {type === 'candlestick' ? (
                       ...
                       data.map((candle, index) => {
                           ...
                           const openY = padding.top + innerHeight - ((candle.open - minPrice) / priceRange) * innerHeight;
                           const closeY = padding.top + innerHeight - ((candle.close - minPrice) / priceRange) * innerHeight;
                           const highY = padding.top + innerHeight - ((candle.high - minPrice) / priceRange) * innerHeight;
                           const lowY = padding.top + innerHeight - ((candle.low - minPrice) / priceRange) * innerHeight;
                       ...
                       <>
                           ...
                           <path
                               d={`M ${padding.left} ${padding.top + innerHeight} ${data.map((point, index) => {
                                   const x = padding.left + (index * innerWidth) / (data.length - 1);
           ...
           <div className="max-w-7xl mx-auto">
               ...
               <div className="bg-slate-800 rounded-lg p-6 mb-6">
                   <div className="flex items-center justify-between">
                       <div className="flex items-center gap-4">
                           ...
                           />
                           <div>
                               <h1 className="text-3xl font-bold text-white">{tokenDetails.name}</h1>
                               <p className="text-slate-400 text-lg uppercase">{tokenDetails.symbol}</p>
                           </div>
                       </div>
                       <div className="text-right">
                           ...
                           <div className={`text-lg font-semibold ${tokenDetails.price_change_percentage_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                       ...
                       <div className="bg-slate-800 rounded-lg p-6 mb-6">
                           <div className="grid grid-cols-4 gap-6">
                               <div>
                                   ...
                                   <p className={`text-lg font-semibold ${tokenDetails.price_change_percentage_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                       {formatPercentage(tokenDetails.price_change_percentage_24h)}
                                   </p>
                               </div>
                               <div>
                   ...
                   <div className="space-y-6">
                       ...
                       <div className="bg-slate-800 rounded-lg p-6">
                           <h3 className="text-xl font-bold mb-4">About {tokenDetails.name}</h3>
                           <p className="text-slate-300 leading-relaxed">
                               {tokenDetails.name} is a cryptocurrency that operates on a decentralized network. 
                               It enables peer-to-peer transactions and is secured by cryptographic protocols. 
                               The network processes transactions through a distributed ledger system.
                           </p>
                       </div>
                       ...
                       <button className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg transition-colors flex items-center justify-center gap-2">
                           <span className="text-xl">+</span>
                           Add to Portfolio
                       </button>
                   </div>
