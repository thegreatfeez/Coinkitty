...
import { Calendar, ExternalLink, BookOpen, TrendingUp, Users } from 'lucide-react'; 
...
export default function Learn() {
    ...
    const categorizeContent = (title, description) => {
        ...
        if (text.includes('beginner') || text.includes('guide') || text.includes('how to') || text.includes('tutorial')) {
            return 'beginner';
        } else if (text.includes('analysis') || text.includes('market') || text.includes('trading')) {
            return 'analysis';
        } else if (text.includes('technology') || text.includes('blockchain') || text.includes('defi')) {
            return 'technology';
        } else if (text.includes('regulation') || text.includes('legal') || text.includes('government')) {
            return 'regulatory';
        }
        return 'general';
    };
    ...
    React.useEffect(() => {
        async function getNews() {
            try{
                ...
                const allContent = [
                    ...newsArticle.map(article => ({...article, category: 'news',
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 py-12">
                <div className="max-w-4xl mx-auto px-6">
                    ...
                    <p className="text-xl text-blue-100">
                        Stay updated with the latest crypto news, educational content, and market insights
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-6 py-8">
                ...
                <div className="flex flex-wrap gap-2 mb-8 border-b border-slate-700">
                    {[
                        ...
                        { id: 'beginner', label: 'Beginner Guides', count: news.filter(n => n.type === 'beginner').length },
                        { id: 'analysis', label: 'Market Analysis', count: news.filter(n => n.type === 'analysis').length },
                        { id: 'technology', label: 'Technology', count: news.filter(n => n.type === 'technology').length },
                        { id: 'regulatory', label: 'Regulatory', count: news.filter(n => n.type === 'regulatory').length }
                    ].map(tab => (
                        <button
                            key={tab.id}
                ...
                {loading ? (
                    ...
                    <div className="grid gap-6">
                        {filterContent(news).slice(0, 20).map((article, index) => (
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className={`px-2 py-1 text-xs rounded-full ${
                                                article.type === 'beginner' ? 'bg-green-900 text-green-300' :
                                                article.type === 'analysis' ? 'bg-blue-900 text-blue-300' :
                                                article.type === 'technology' ? 'bg-purple-900 text-purple-300' :
